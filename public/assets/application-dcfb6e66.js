...
%o`, message, error2, detail);
  ...
  // app/javascript/controllers/fruitbox_controller.js
  var fruitbox_controller_default = class extends Controller {
    ...
    initializeGame() {
      ...
      this.selectedTiles = /* @__PURE__ */ new Set();
    ...
    checkAndDestroySelection() {
      ...
      for (let i = minRow; i <= maxRow; i++) {
        for (let j = minCol; j <= maxCol; j++) {
          if (this.board[i][j] && this.board[i][j].fruit === "\u{1F34E}") {
    ...
    showNewBoardMessage() {
      ...
      messageElement.textContent = "No more moves! New board generated. \u{1F389}";
      setTimeout(() => {
        if (messageElement) {
  ...
  // app/javascript/controllers/multiplayer_fruitbox_controller.js
  console.log("Multiplayer FruitBox controller file loaded!");
  var multiplayer_fruitbox_controller_default = class extends Controller {
    static targets = ["board"];
    static values = {
      roomCode: String,
      playerId: Number,
    ...
    async loadBoard() {
      try {
        ...
        console.error("Error loading board:", error2);
    ...
    async refreshBoard() {
      try {
        console.log("Refreshing board for player", this.playerIdValue);
        const response = await fetch(`/game/board/${this.roomCodeValue}/refresh?player_id=${this.playerIdValue}`, {
          method: "POST",
          headers: {
        ...
        const data = await response.json();
        console.log("New board data received:", data);
        if (data.board) {
          this.renderBoard(data.board);
        } else {
          console.error("No board data in response");
        ...
        console.error("Error refreshing board:", error2);
      }
    }
    generateTestBoard() {
      const board = [];
    ...
    async checkAndDestroySelection() {
      ...
      for (let i = minRow; i <= maxRow; i++) {
        for (let j = minCol; j <= maxCol; j++) {
          ...
          if (tile && tile.querySelector("div:first-child").textContent === "\u{1F34E}") {
      ...
      if (sum === 10 && selectedPositions.length > 0) {
        try {
          ...
          console.error("Error collecting apples:", error2);
        }
      }
    }
    updateScore() {
    ...
    async endGame() {
      ...
      try {
        ...
        console.error("Error notifying server of game completion:", error2);
      }
    ...
    async checkGameCompletion() {
      this.completionCheckInterval = setInterval(async () => {
        try {
          ...
          console.error("Error checking game completion:", error2);
        }
      }, 3e3);
    }
    ...
    toggleHint() {
      ...
      if (hintButton) {
        if (this.hintMode) {
          ...
          hintButton.className = "px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700";
        } else {
          hintButton.textContent = "Show Hints";
          hintButton.className = "px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700";
        }
      }
    }
    showHints() {
      ...
      legalMoves.forEach((move, index) => {
        const hintBox = document.createElement("div");
        hintBox.className = "absolute border-2 border-green-500 bg-green-100 bg-opacity-30 pointer-events-none z-20";
        const minRow = Math.min(...move.map((pos) => pos.row));
        const maxRow = Math.max(...move.map((pos) => pos.row));
        const minCol = Math.min(...move.map((pos) => pos.col));
        const maxCol = Math.max(...move.map((pos) => pos.col));
        const tileSize = 48;
        const gap = 4;
        const left = minCol * (tileSize + gap);
        const top = minRow * (tileSize + gap);
    ...
    hideHints() {
      this.hintBoxes.forEach((box) => box.remove());
      this.hintBoxes = [];
    }
    findLegalMoves() {
      const legalMoves = [];
      ...
      for (let i = 0; i < 10; i++) {
        for (let j = 0; j < 10; j++) {
          ...
          if (tile && tile.querySelector("div:first-child").textContent === "\u{1F34E}") {
            const value = parseInt(tile.querySelector("div:last-child").textContent);
            availableApples.push({ row: i, col: j, value });
          }
        }
      }
      this.findLegalCombinations(availableApples, 10, [], 0, legalMoves, 20);
      return legalMoves;
    }
    findLegalCombinations(apples, targetSum, currentCombination, startIndex, legalMoves, maxMoves) {
      const currentSum = currentCombination.reduce((sum, apple) => sum + apple.value, 0);
      if (currentSum === targetSum && currentCombination.length > 0) {
        legalMoves.push([...currentCombination]);
        return;
      }
      if (currentSum > targetSum || legalMoves.length >= maxMoves) {
        return;
      }
      for (let i = startIndex; i < apples.length && legalMoves.length < maxMoves; i++) {
        currentCombination.push(apples[i]);
        this.findLegalCombinations(apples, targetSum, currentCombination, i + 1, legalMoves, maxMoves);
        currentCombination.pop();
      }
    }
    testConnection() {
      console.log("Test button clicked!");
      alert("Controller is working! Room: " + this.roomCodeValue + ", Player: " + this.playerIdValue);
      const statusElement = document.getElementById("controller-status");
      if (statusElement) {
        statusElement.textContent = "Controller test successful!";
        statusElement.className = "text-sm text-green-500 mb-2";
      }
    }
    async fetchBoard() {
  ...
  application.register("fruitbox", fruitbox_controller_default);
  application.register("multiplayer-fruitbox", multiplayer_fruitbox_controller_default);
})();
...
@hotwired/turbo/dist/turbo.es2017-esm.js:
   ...
   *)
*/
//# sourceMappingURL=/assets/application-1cd7890a.js.map
