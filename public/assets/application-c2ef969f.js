  ...
  var multiplayer_fruitbox_controller_default = class extends Controller {
    ...
    showHints() {
      ...
      solutions.forEach((solution, index) => {
        ...
        const minRow = Math.min(...solution.map((pos) => pos.row));
        const maxRow = Math.max(...solution.map((pos) => pos.row));
        const minCol = Math.min(...solution.map((pos) => pos.col));
        const maxCol = Math.max(...solution.map((pos) => pos.col));
        const tileSize = 48;
        const gap = 4;
        const left = minCol * (tileSize + gap);
        const top = minRow * (tileSize + gap);
        ...
        label.className = "absolute -top-6 -left-1 bg-green-500 text-white text-xs px-1 rounded";
    ...
    findAllSolutions() {
      ...
      this.findCombinations(availableApples, 10, [], 0, solutions);
      return solutions;
    }
    findCombinations(apples, targetSum, currentCombination, startIndex, solutions) {
      ...
      for (let i = startIndex; i < apples.length; i++) {
        currentCombination.push(apples[i]);
        this.findCombinations(apples, targetSum, currentCombination, i + 1, solutions);
...
@hotwired/turbo/dist/turbo.es2017-esm.js:
   ...
   *)
*/
//# sourceMappingURL=/assets/application-37cc3aad.js.map
